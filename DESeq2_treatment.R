#!/usr/bin/env Rscript
setwd("/Users/ap14958/OneDrive - University of Bristol/Genomics Facility Bioinformatics/Project #13 Sus_scrofa - Melanie Hazzell/")
...
ddsHTSeq$phase2_cat=relevel(ddsHTSeq$treatment, ref = reference_in)
levels(ddsHTSeq$timepoint)
levels(ddsHTSeq$treatment)
...
# 
# # ddsHTSeq$phase2_cat
# levels =levels(ddsHTSeq$treatment)
...
# # 
# res1 = results(ddskeep,contrast=c("timepoint","baseline","termination"), alpha = 0.05)
# res1
# write.csv(res1[order(res1["padj"]),], file = "DESeq2_Pairwise_OutputTables/DESeq2_baseline_vs_termination.csv")
...
dds_lrt <- DESeq(ddsHTSeq, test = "LRT", reduced = ~timepoint)
...
res1_lrt = results(ddskeep_lrt,contrast=c("treatment","graft","control"), alpha = 0.05)
write.csv(res1_lrt[order(res1_lrt["padj"]),], file = "DESeq2_LRT_OutputTables/DESeq2_control_vs_graft.csv")
...
fiss <- plotCounts(dds_lrt, gene=rownames(top), 
                   intgroup = "timepoint", returnData = T)
most_sig = ggplot(fiss,
                  aes(x = treatment, y = count, color = treatment, group = 1)) + 
...
# # 
# gene_to_test = "PON3"
# gene = plotCounts(dds_lrt,gene_to_test, returnData = T, normalized = T, intgroup = "treatment")
# # 
# plot = ggplot(gene,
...
#PCA Plots
library(ggplot2)
PCA=plotPCA(vsd, intgroup = c("treatment"), ntop = 100000, returnData = TRUE)
percentVar <- round(100 * attr(PCA, "percentVar"))

PCA2 = ggplot(data = PCA, aes(PC1, PC2, color = treatment, label = row.names(PCA))) +
  geom_point(size=3) +
  geom_text(size=3) +
  geom_label() +
...
volcano_lrt = EnhancedVolcano(res1_lrt,
                ...
                selectLab = rownames(res1_lrt)[res1_lrt$padj <= 0.000000001], #label 20 most signifcant ASVs
                drawConnectors = TRUE, 
...
ColourStrategy = metadata$treatment
