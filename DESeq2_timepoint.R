...
dds_lrt <- DESeq(ddsHTSeq, test = "LRT", reduced = ~treatment)
...
res1_lrt = results(ddskeep_lrt,contrast=c("timepoint","termination","baseline"), alpha = 0.05)
write.csv(res1_lrt[order(res1_lrt["padj"]),], file = "DESeq2_LRT_OutputTables/DESeq2_baseline_vs_termination.csv")
...
most_sig = ggplot(fiss,
                  aes(x = timepoint, y = count, color = timepoint, group = 1)) + 
...
# 
gene_to_test = "PON3"
gene = plotCounts(dds_lrt,gene_to_test, returnData = T, normalized = T, intgroup = "timepoint")
# 
plot = ggplot(gene,
...
#PCA Plots
library(ggplot2)
PCA=plotPCA(vsd, intgroup = c("timepoint"), ntop = 100000, returnData = TRUE)
percentVar <- round(100 * attr(PCA, "percentVar"))

PCA2 = ggplot(data = PCA, aes(PC1, PC2, color = timepoint, label = row.names(PCA))) +
  geom_point(size=3) +
  geom_text(size=3) +
  geom_label() +
